...
const char HTML[] =
		...
		"                data = eval('(' + data + ')');\n"
		"\n"
		//"                var headers = [\"File size\", \"Backend\", \"Requests\", \"HTTP 499\", \"HTTP 500\", \"HTTP 503\", \"TCP errors\", \"HTTP<br/>read timeouts\",\n"
		//"                               \"HTTP<br/>write timeouts\", \"Fail timeouts, sec.\", \"Max fails\", \"Last fail\" ];\n"
		"                var headers = [\"File size\", %s ];\n"
		"\n"
...
static ngx_int_t ngx_http_filestats_handler(ngx_http_request_t *r)
{
    ...
    ngx_buf_t *b = NULL;
    ngx_chain_t out;
   // ngx_http_get_module_loc_conf(r, ngx_http_filestats_module);
